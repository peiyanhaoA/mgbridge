...
(function make_xlsx(XLSX) {
    ...
    var make_ssf = function make_ssf(SSF) {
        ...
        function _strrev(x) { var o = "",
                i = x.length - 1; while (i >= 0) o += x.charAt(i--); return o; }
            ...
            while (i < fmt.length) {
                switch ((c = fmt.charAt(i))) {
                    ...
                    case 'a':
                        ...
                        if (fmt.substr(i, 3).toUpperCase() === "A/P") { if (dt != null) q.v = dt.H >= 12 ? "P" : "A";
                            q.t = 'T';
                            hr = 'h';
                            i += 3; } else if (fmt.substr(i, 5).toUpperCase() === "AM/PM") { if (dt != null) q.v = dt.H >= 12 ? "PM" : "AM";
                            q.t = 'T';
                            i += 5;
                            hr = 'h'; } else { q.t = "t";++i; }
    ...
    var CFB = (function _CFB() {
        ...
        function parse(file, options) {
            ...
            /* reprocess header */
            if (ssz !== 512) { blob = file.slice(0, ssz);
        ...
        function read_directory(dir_start, sector_list, sectors, Paths, nmfs, files, FileIndex, mini) {
            ...
            for (; i < sector.length; i += 128) {
                ...
                if (o.size < 0 && o.start < 0) { o.size = o.type = 0;
                    o.start = ENDOFCHAIN;
                    o.name = ""; }
                if (o.type === 5) { /* root */
        ...
        function rebuild_cfb(cfb, f) {
            ...
            cfb.FullPaths = [];
            cfb.FileIndex = [];
            for (i = 0; i < data.length; ++i) { cfb.FullPaths[i] = data[i][0];
        ...
        function find(cfb, path) {
            ...
            var k = false;
            if (path.charCodeAt(0) === 47 /* "/" */ ) { k = true;
                path = UCFullPaths[0].slice(0, -1) + path; } else k = path.indexOf("/") !== -1;
    ...
    function split_regex(str, re, def) {
        ...
        var p = str.split(re),
            o = [p[0]];
        for (var i = 1; i < p.length; ++i) { o.push(def);
            o.push(p[i]); }
        return o;
    }
    ...
    var utf8read = function utf8reada(orig) {
        ...
        while (i < orig.length) {
            ...
            d = orig.charCodeAt(i++);
            if (c > 191 && c < 224) { f = ((c & 31) << 6);
                f |= (d & 63);
                out += String.fromCharCode(f); continue; }
            e = orig.charCodeAt(i++);
    ...
    function write_double_le(b, v, idx) {
        var bs = ((v < 0 || 1 / v == -Infinity) ? 1 : 0) << 7,
            ...
            m = 0;
        var av = bs ? -v : v;
        if (!isFinite(av)) { e = 0x7ff;
            m = isNaN(v) ? 0x6969 : 0; } else if (av == 0) e = m = 0;
        else {
            e = Math.floor(Math.log(av) / Math.LN2);
            m = av * Math.pow(2, 52 - e);
            if (e <= -1023 && (!isFinite(m) || m < Math.pow(2, 52))) { e = -1022; } else { m -= Math.pow(2, 52);
                e += 1023; }
        }
    ...
    function ReadShift(size, t) {
        ...
        switch (t) {
            ...
            case '_wstr':
                size = 0;
                o = "";
                while ((w = __readUInt16LE(this, this.l + size)) !== 0) { oo.push(_getchar(w));
                    size += 2; }
                size += 2;
                o = oo.join("");
                break;
            ...
            default:
                switch (size) {
                    ...
                    case 4:
                    case -4:
                        if (t === 'i' || (this[this.l + 3] & 0x80) === 0) { oI = (size > 0 ? __readInt32LE : __readInt32BE)(this, this.l);
                            this.l += 4; return oI; } else { oR = __readUInt32LE(this, this.l);
                            this.l += 4; }
                        return oR;
                    case 8:
                    case -8:
    ...
    var __writeUInt32LE = function(b, val, idx) { b[idx] = (val & 0xFF);
        b[idx + 1] = ((val >>> 8) & 0xFF);
        b[idx + 2] = ((val >>> 16) & 0xFF);
        b[idx + 3] = ((val >>> 24) & 0xFF); };
    var __writeInt32LE = function(b, val, idx) { b[idx] = (val & 0xFF);
        b[idx + 1] = ((val >> 8) & 0xFF);
        b[idx + 2] = ((val >> 16) & 0xFF);
        b[idx + 3] = ((val >> 24) & 0xFF); };
    var __writeUInt16LE = function(b, val, idx) { b[idx] = (val & 0xFF);
        b[idx + 1] = ((val >>> 8) & 0xFF); };
    ...
    function parsenooplog(blob, length) { if (typeof console != 'undefined') console.log(blob.slice(blob.l, blob.l + length));
        blob.l += length; }
    ...
    function write_record(ba, type, payload, length) {
        ...
        for (var i = 0; i != 4; ++i) {
            if (length >= 0x80) { o.write_shift(1, (length & 0x7F) + 0x80);
                length >>= 7; } else { o.write_shift(1, length); break; }
        }
    ...
    function decode_col(colstr) { var c = unfix_col(colstr),
            d = 0,
    ...
    function aoa_to_sheet(data, opts) {
        ...
        for (var R = 0; R != data.length; ++R) {
            for (var C = 0; C != data[R].length; ++C) {
                ...
                var cell = ({ v: data[R][C] });
                if (Array.isArray(cell.v)) { cell.f = data[R][C][1];
                    cell.v = cell.v[0]; }
                ...
                if (cell.v === null) { if (cell.f) cell.t = 'n';
                    ...
                    if (o.cellDates) { cell.t = 'd';
                        cell.w = SSF.format(cell.z, datenum(cell.v)); } else { cell.t = 'n';
    ...
    function write_RkNumber(data, o) {
        ...
        var fX100 = 0,
            fInt = 0,
            d100 = data * 100;
        if (data == (data | 0) && data >= -(1 << 29) && data < (1 << 29)) { fInt = 1; } else if (d100 == (d100 | 0) && d100 >= -(1 << 29) && d100 < (1 << 29)) { fInt = 1;
            fX100 = 1; }
    ...
    function write_BrtColor(color, o) {
        if (!o) o = new_buf(8);
        if (!color || color.auto) { o.write_shift(4, 0);
            o.write_shift(4, 0); return o; }
        if (color.index) {
    ...
    function xlml_write_custprops(Props, Custprops, opts) {
        ...
        if (Props) keys(Props).forEach(function(k) {
            ...
            var m = Props[k];
            var t = "string";
            if (typeof m == 'number') { t = "float";
                m = String(m); } else if (m === true || m === false) { t = "boolean";
                m = m ? "1" : "0"; } else m = String(m);
        ...
        if (Custprops) keys(Custprops).forEach(function(k) {
            ...
            if (typeof m == 'number') { t = "float";
                m = String(m); } else if (m === true || m === false) { t = "boolean";
                m = m ? "1" : "0"; } else if (m instanceof Date) { t = "dateTime.tz";
                m = m.toISOString(); } else m = String(m);
            o.push(writextag(escapexmltag(k), m, { "dt:dt": t }));
        });
        ...
        for (i = 0; i != NumProps; ++i) {
            if (blob.l !== Props[i][1]) {
                ...
                if (i > 0 && PIDSI) switch (PIDSI[Props[i - 1][0]].t) {
                    case 0x02 /*VT_I2*/ :
                        if (blob.l + 2 === Props[i][1]) { blob.l += 2;
                            fail = false; }
                        break;
                    case 0x50 /*VT_STRING*/ :
                        if (blob.l <= Props[i][1]) { blob.l = Props[i][1];
                            fail = false; }
                        break;
                    case 0x100C /*VT_VECTOR|VT_VARIANT*/ :
                        if (blob.l <= Props[i][1]) { blob.l = Props[i][1];
                ...
                if (!PIDSI && blob.l <= Props[i][1]) { fail = false;
                    blob.l = Props[i][1]; }
            ...
            if (PIDSI) {
                ...
                if (Props[i][0] === 0x1) {
                    ...
                    if (CodePage === 0) { Dictionary = i;
                        blob.l = Props[i + 1][1]; continue; }
                    DictObj = parse_dictionary(blob, CodePage);
                } else {
    ...
    function parse_PropertySetStream(file, PIDSI) {
        ...
        else if (NumSets === 2) { FMTID1 = blob.read_shift(16);
    ...
    function writebool(v, o) { if (!o) o = new_buf(2);
        o.write_shift(2, +!!v); return o; }

    function parseuint16(blob) { return blob.read_shift(2, 'u'); }

    function writeuint16(v, o) { if (!o) o = new_buf(2);
        o.write_shift(2, v); return o; }
    ...
    /* 2.5.178 LongRGBA */
    function parse_LongRGBA(blob, length) { var r = blob.read_shift(1),
            g = blob.read_shift(1),
            b = blob.read_shift(1),
            a = blob.read_shift(1); return [r, g, b, a]; }

    /* 2.5.177 LongRGB */
    function parse_LongRGB(blob, length) { var x = parse_LongRGBA(blob, length);
        x[3] = 0; return x; }
    ...
    function parse_FtSkip(blob, length) { blob.l += 2;
    ...
    function parse_BOF(blob, length) {
        ...
        length -= 2;
        if (length >= 2) { o.dt = blob.read_shift(2);
            blob.l -= 2; }
        switch (o.BIFFVer) {
            case 0x0600:
                /* BIFF8 */
                    ...
                    case 'F':
                        ...
                        for (rj = 1; rj < record.length; ++rj) switch (record[rj].charAt(0)) {
                            ...
                            case 'R':
                                ...
                                if (Mval > 0) { rowinfo[R].hpt = Mval;
                                    rowinfo[R].hpx = pt2px(Mval); } else if (Mval === 0) rowinfo[R].hidden = true;
                                break;
                            default:
                                if (opts && opts.WTF) throw new Error("SYLK bad record " + rstr);
                        }
        ...
        function dif_to_aoa_str(str, opts) {
            ...
            for (; ri !== records.length; ++ri) {
                ...
                switch (+type) {
                    case -1:
                        if (data === 'BOT') { arr[++R] = [];
                            C = 0; continue; } else if (data !== 'EOD') throw new Error("Unrecognized DIF special command " + data);
                        break;
                    case 0:
    ...
    var PRN = (function() {
        ...
        function dsv_to_sheet_str(str, opts) {
            ...
            if (str.substr(0, 4) == "sep=" && str.charCodeAt(5) == 10) { sep = str.charAt(4);
                str = str.substr(6); } else sep = guess_sep(str.substr(0, 1024));
            var R = 0,
                C = 0,
                v = 0;
            var start = 0,
            ...
            function finish_cell() {
                ...
                else if (o.raw) { cell.t = 's';
                    cell.v = s; } else if (s.trim().length === 0) { cell.t = 's';
                    cell.v = s; } else if (s.charCodeAt(0) == 0x3D) {
                    if (s.charCodeAt(1) == 0x22 && s.charCodeAt(s.length - 1) == 0x22) { cell.t = 's';
                        cell.v = s.slice(2, -1).replace(/""/g, '"'); } else if (fuzzyfmla(s)) { cell.t = 'n';
                        cell.f = s.substr(1); } else { cell.t = 's';
                        cell.v = s; }
                } else if (s == "TRUE") { cell.t = 'b';
                    cell.v = true; } else if (s == "FALSE") { cell.t = 'b';
                    cell.v = false; } else if (!isNaN(v = fuzzynum(s))) { cell.t = 'n'; if (o.cellText !== false) cell.w = s;
                    cell.v = v; } else if (!isNaN(fuzzydate(s).getDate()) || _re && s.match(_re)) {
                    cell.z = o.dateNF || SSF._table[14];
                    var k = 0;
                    if (_re && s.match(_re)) { s = dateNF_fix(s, o.dateNF, (s.match(_re) || []));
                        k = 1; }
                    if (o.cellDates) { cell.t = 'd';
                        cell.v = parseDate(s, k); } else { cell.t = 'n';
                ...
                if (cell.t == 'z') {} else if (o.dense) { if (!ws[R]) ws[R] = [];
                    ws[R][C] = cell; } else ws[encode_cell({ c: C, r: R })] = cell;
                start = end + 1;
                if (range.e.c < C) range.e.c = C;
                if (range.e.r < R) range.e.r = R;
                if (cc == sepcc) ++C;
    ...
    var WK_ = (function() {
        ...
        function lotus_to_workbook_buf(d, opts) {
            ...
            else if (d[2] == 0x0e) { o.Enum = WK3Enum;
                o.qpro = true;
                d.l = 0; } else throw new Error("Unrecognized LOTUS BOF " + d[2]);
    ...
    var parse_rs = (function parse_rs_factory() {
        ...
        var parse_rpr = function parse_rpr(rpr, intro, outro) {
            ...
            if (font.b) { intro.push("<b>");
                outro.push("</b>"); }
            if (font.i) { intro.push("<i>");
                outro.push("</i>"); }
            if (font.strike) { intro.push("<s>");
                outro.push("</s>"); }

            if (align == "superscript") align = "sup";
            else if (align == "subscript") align = "sub";
            if (align != "") { intro.push("<" + align + ">");
                outro.push("</" + align + ">"); }
    ...
    function find_mdw_colw(collw) {
        ...
        if (delta > 0.005)
            for (MDW = MIN_MDW; MDW < MAX_MDW; ++MDW)
                if (Math.abs(collw - cycle_width(collw)) <= delta) { delta = Math.abs(collw - cycle_width(collw));
                    _MDW = MDW; }
        MDW = _MDW;
    }
    ...
    function find_mdw_wpx(wpx) {
        ...
        for (MDW = MIN_MDW; MDW < MAX_MDW; ++MDW) {
            ...
            guess = (guess) % 1;
            if (guess > 0.5) guess--;
            if (Math.abs(guess) < delta) { delta = Math.abs(guess);
                _MDW = MDW; }
        }
        MDW = _MDW;
    }
	...
	*/
    ...
    function write_comments_xml(data, opts) {
        ...
        o.push("</commentList>");
        if (o.length > 2) { o[o.length] = ('</comments>');
            o[1] = o[1].replace("/>", ">"); }
        return o.join("");
    }
        ...
        for (var ff = 0, fflen = formula[0].length; ff < fflen; ++ff) {
            ...
            switch (f[0]) {
                ...
                case 'PtgMemFunc':
                    ...
                    break;
                case 'PtgMemNoMem':
                    /* 2.5.198.73 TODO -- find a test case */
                ...
                case 'PtgAttrBaxcel':
                    /* 2.5.198.33 TODO -- find a test case*/
                    throw new Error('Unrecognized Formula Token: ' + String(f));
                case 'PtgSxName':
                    /* 2.5.198.91 TODO -- find a test case */
                    throw new Error('Unrecognized Formula Token: ' + String(f));
                case 'PtgList':
                    /* [MS-XLSB] 2.5.97.52 TODO -- find a test case */
                    throw new Error('Unrecognized Formula Token: ' + String(f));
    ...
    function col_obj_w(C, col) {
        ...
        if (wch > -1) { p.width = char2width(wch);
            p.customWidth = 1; } else if (col.width != null) p.width = col.width;
    ...
    function parse_ws_xml_hlinks(s, data, rels) {
        ...
        for (var i = 0; i != data.length; ++i) {
            ...
            if (val.tooltip) { val.Tooltip = val.tooltip;
                delete val.tooltip; }
        ...
        return function parse_ws_xml_data(sdata, s, opts, guess, themes, styles) {
            ...
            for (var marr = sdata.split(rowregex), mt = 0, marrlen = marr.length; mt != marrlen; ++mt) {
                ...
                if (opts && opts.cellStyles) {
                    rowobj = {};
                    rowrite = false;
                    if (tag.ht) { rowrite = true;
                        rowobj.hpt = parseFloat(tag.ht);
                        rowobj.hpx = pt2px(rowobj.hpt); }
                    if (tag.hidden == "1") { rowrite = true;
                        rowobj.hidden = true; }
                    if (tag.outlineLevel != null) { rowrite = true;
                ...
                for (ri = 0; ri != cells.length; ++ri) {
                    ...
                    switch (p.t) {
                        ...
                        case 'd':
                            ...
                            else { p.v = datenum(parseDate(p.v, 1));
                                p.t = 'n'; }
                            break;
                    ...
                    if (opts.cellDates && do_format && p.t == 'n' && SSF.is_date(SSF._table[fmtid])) { p.t = 'd';
                        p.v = numdate(p.v); }
                    if (dense) {
        ...
        recordhopper(data, function ws_parse(val, R_n, RT) {
            ...
            switch (RT) {
                ...
                case 0x000B:
                    ...
                    C = val[0].c;
                    if (opts.dense) { if (!s[R]) s[R] = [];
                        s[R][C] = p; } else s[encode_col(C) + rr] = p;
                    if (opts.cellFormula) {
                        af = false;
    ...
    function write_cs_xml(idx, opts, wb, rels) {
        ...
        if (o.length > 2) { o[o.length] = ('</chartsheet>');
            o[1] = o[1].replace("/>", ">"); }
        return o.join("");
    }
    ...
    function parse_wb_bin(data, opts) {
        ...
        recordhopper(data, function hopper_wb(val, R_n, RT) {
            switch (RT) {
                ...
                case 0x016A:
                    /* 'BrtExternSheet' */
                    if (supbooks.length === 0) { supbooks[0] = [];
                        supbooks[0].XTI = []; }
        ...
        while ((Rn = xlmlregex.exec(str))) switch (Rn[3]) {
            ...
            case 'Cell':
                if (Rn[1] === '/') {
                    ...
                    if (!opts.sheetStubs) { if (cell.MergeAcross) c = cc + 1;
                        else ++c; } else if (cell.MergeAcross || cell.MergeDown) {
            ...
            case 'Row':
                if (Rn[1] === '/' || Rn[0].slice(-2) === "/>") {
                    ...
                    if (row.Hidden == "1") { rowobj.hidden = true;
                        rowinfo[r] = rowobj; }
                }
                break;
            case 'Worksheet':
            ...
            case 'Column':
                ...
                if (csty.Hidden) { csty.hidden = true;
                    delete csty.Hidden; }
    ...
    */
    function slurp(R, blob, length, opts) {
        ...
        var ll = 0;
        b.lens = [];
        for (var j = 0; j < bufs.length; ++j) { b.lens.push(ll);
            ll += bufs[j].length; }
                    ...
                    default:
                        switch (R.n) { /* nested */
                            ...
                            case 'Row':
                                {
                                    var rowobj = {};
                                    if (val.level != null) { rowinfo[val.r] = rowobj;
                                        rowobj.level = val.level; }
                                    if (val.hidden) { rowinfo[val.r] = rowobj;
                                        rowobj.hidden = true; }
                                    if (val.hpt) {
    ...
    function write_BIFF2BERR(r, c, val, t) {
        var out = new_buf(9);
        write_BIFF2Cell(out, r, c);
        if (t == 'e') { out.write_shift(1, val);
            out.write_shift(1, 1); } else { out.write_shift(1, val ? 1 : 0);
            out.write_shift(1, 0); }
        return out;
    }
    ...
    function write_ws_biff8_cell(ba, cell, R, C, opts) {
        if (cell.v != null) switch (cell.t) {
            ...
            case 'b':
            case 'e':
                write_biff_rec(ba, "BoolErr", write_BoolErr(R, C, cell.v, opts, cell.t));
                return;
                /* TODO: codepage, sst */
            case 's':
            case 'str':
        ...
        function html_to_sheet(str, _opts) {
            ...
            for (i = 0; i < rows.length; ++i) {
                ...
                for (j = 0; j < cells.length; ++j) {
                    ...
                    if (opts.dense) {
                        if (!ws[R]) ws[R] = [];
                        if (!m.length) {} else if (opts.raw || !m.trim().length) ws[R][C] = { t: 's', v: m };
                        ...
                        var coord = encode_cell({ r: R, c: C });
                        /* TODO: value parsing */
                        if (!m.length) {} else if (opts.raw) ws[coord] = { t: 's', v: m };
        ...
        for (; R < rows.length; ++R) {
            ...
            for (_C = C = 0; _C < elts.length; ++_C) {
                ...
                for (midx = 0; midx < merges.length; ++midx) {
                    var m = merges[midx];
                    if (m.s.c == C && m.s.r <= R && R <= m.e.r) { C = m.e.c + 1;
                        midx = -1; }
                }
                ...
                if (v != null) {
                    if (v.length == 0) o.t = 'z';
                    else if (opts.raw) {} else if (v.trim().length == 0) o.t = 's';
                ...
                if (opts.dense) { if (!ws[R]) ws[R] = [];
                    ws[R][C] = o; } else ws[encode_cell({ c: C, r: R })] = o;
                if (range.e.c < C) range.e.c = C;
                C += CS;
            }
        }
        ...
        return function pcx(d, _opts) {
            ...
            while ((Rn = xlmlregex.exec(str))) switch ((Rn[3] = Rn[3].replace(/_.*$/, ""))) {
                ...
                case 'covered-table-cell': // 9.1.5 <table:covered-table-cell>
                    ...
                    if (opts.sheetStubs) {
                        if (opts.dense) { if (!ws[R]) ws[R] = [];
                            ws[R][C] = { t: 'z' }; } else ws[encode_cell({ r: R, c: C })] = { t: 'z' };
                    }
                    break; /* stub */
                case 'table-cell':
                case '数据':
                    if (Rn[0].charAt(Rn[0].length - 2) === '/') {
                        ...
                        if (comments.length > 0) { q.c = comments;
                            comments = []; }
                ...
                case '文本串': // 5.1.3 <text:p>
                    ...
                    else { textptag = parsexmltag(Rn[0], false);
    ...
    function readSync(data, opts) {
        ...
        if (!o.type) o.type = (has_buf && Buffer.isBuffer(data)) ? "buffer" : "base64";
        if (o.type == "file") { o.type = "buffer";
            d = _fs.readFileSync(data); }
        if (o.type == "string") { str = true;
            o.type = "binary";
            d = bstrify(data); }
    ...
    function write_cfb_type(wb, opts) {
        ...
        switch (o.type) {
            ...
            case "array":
                ...
                break;
            case "file":
                return _fs.writeFileSync(o.file, CFB.write(cfb, { type: 'buffer' }));
            case "string":
                throw new Error("'string' output type invalid for '" + o.bookType + ' files');
            default:
                throw new Error("Unrecognized type " + o.type);
    ...
    function write_string_type(out, opts, bom) {
        ...
        switch (opts.type) {
            ...
            case "string":
                return out;
            case "file":
                return _fs.writeFileSync(opts.file, o, 'utf8');
            case "buffer":
                {
    ...
    function sheet_to_csv(sheet, opts) {
        ...
        for (var C = r.s.c; C <= r.e.c; ++C)
            if (!((colInfos[C] || {}).hidden)) cols[C] = encode_col(C);
        for (var R = r.s.r; R <= r.e.r; ++R) {
